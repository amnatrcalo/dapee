package com.forum.springcloud.gateway.filter;
...
import com.forum.springcloud.gateway.req_res.ValidateTokenResponse;
...
import static com.forum.springcloud.gateway.error.ErrorConstants.INVALID_LOGIN_DETAILS;
import static com.forum.springcloud.gateway.error.ErrorConstants.TOKEN_NOT_FOUND;
    ...
    public GatewayFilter apply(Config config) {
        return (((exchange, chain) -> {
            if(routeValidator.isSecured.test(exchange.getRequest())){
                ...
                try{
                    String host = discoveryClient.getInstances("LOGIN").get(0).getUri().toString();
